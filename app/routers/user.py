...
async def create_user(db: Annotated[Session, Depends(get_db)], create_user_: CreateUser):
    db.execute(insert(User).values(
                                username=create_user_.username,       # values подстваляем в табличку значение username и тд.
                                firstname=create_user_.firstname,
                                lastname=create_user_.lastname,
                                age=create_user_.age,
                                slag=slugify(create_user_.username)))           #slag при помощи библиотеки slugify
    db.commit()
    return {
